...
export default function ReviewVerify({ review, platformStyles = {}, onReviewProcessed }) {
  ...
  const platformData = platformStyles[review.platform?.id] || { color: "bg-gray-500", name: "Unknown" };
  const submittedDate = review.submitted_at ? formatDate(new Date(review.submitted_at)) : '';
  
  return (
    <Container key={review.id} className="flex-col items-start p-0 overflow-visible">
